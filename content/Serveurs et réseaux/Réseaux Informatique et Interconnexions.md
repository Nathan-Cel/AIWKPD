...
Tout d’abord, commençons par le LAN, c’est-à-dire le Local Area Network , en français «
réseau local ».
...
Ensuite, voyons le WAN, qui signifie Wide Area Network , ou « réseau étendu ».
...
En réalité, Internet est ce qu’on appelle un réseau de réseaux. Il relie des millions de
réseaux locaux (LAN) et étendus (WAN) grâce à des règles communes, qu’on appelle des
protocoles.

Selon Cloudflare ( How does the Internet work , source : cloudflare.com), ces protocoles
...
C’est ce qu’explique TechTarget ( What is TCP/IP , source : techtarget.com).
...
Ce mécanisme, décrit par Cloudflare ( How does the Internet work , source : cloudflare.com),
...
Lorsque tu envoies un mail depuis ton ordinateur connecté à ton LAN :
    ...
    Ton message est transformé en paquets.
    Ces paquets sont transmis par ton routeur.
    Ils traversent plusieurs WAN via des opérateurs.
    Ils arrivent finalement sur un autre LAN, par exemple celui des serveurs Gmail.
    TCP se charge ensuite de reconstituer correctement le message pour ton
    destinataire.
...
D’après l’Internet Society ( A Brief History of the Internet , source : internetsociety.org), c’est
...
Source : étude par l’Université de Quebec, CESI
(segmentation-reseau-v1-2-20240501.pdf)
...
pour les distinguer.
    ...
    Pourquoi le NAT/PAT est important?
...
connecter simultanément à Internet.
    ...
    Fonctionnement simplifié
        Ton PC (192.168.1.2) envoie un paquet vers Internet.
        Le routeur remplace l’adresse privée par l’adresse publique (ex. 102.45.22.1).
        Pour savoir à qui renvoyer la réponse, le routeur garde une table de
        correspondance.
        Quand la réponse arrive, il la renvoie au bon PC.
...
fonctions principales :
    ...
    Déterminer le chemin : choisir la meilleure route selon des critères (délai, capacité,
    fiabilité).
    Transférer les données : envoyer les paquets au prochain routeur ou à
    l’équipement final.
    Équilibrer la charge : répartir le trafic sur plusieurs chemins pour éviter la
    congestion.
